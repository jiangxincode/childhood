...
- [x] 1. 创建游戏核心逻辑（game.js）
  - [x] 1.1 定义棋子名称列表和等级映射（PIECE_NAMES、RANK_MAP），大黄猫 rank=0 为最强
  ...
  - [x] 1.5 实现 `canCapture(attacker, defender)` 吃子规则：rank 数值小的吃大的，同阵营不能吃
  ...
  - [x] 1.15 实现 `aiDecide(state, aiTeam)` AI决策：优先吃牌（优先吃高等级，避免高等级同归于尽）> 翻牌 > 走牌
...
- [x] 2. 创建游戏页面结构（index.html）
  ...
  - [x] 2.5 在规则面板中列出等级排列（大黄猫 > 小花猫 > ... > 油滑鼠）、同级同归于尽、基本操作说明
...
- [x] 5. 更新首页和 README
  - [x] 5.1 在根目录 `index.html` 的 `.gallery_wrap` 中添加猫捉老鼠游戏卡片，链接到 `apps/card-game/cat-and-mouse/index.html`
  - [x] 5.2 在根目录 `README.md` 的游戏列表中添加猫捉老鼠游戏说明条目
