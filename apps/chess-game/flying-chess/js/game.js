...
var TYPE_NAMES = { normal: '玩家', computer: '电脑', close: '无' };
var WIN_COUNT = 4;
...
var START_COORD = { red: 0, blue: 13, yellow: 26, green: 39 };

// Finish channel ranges per color
var FINISH_CHANNEL = { red: { min: 61, max: 66 }, blue: { min: 71, max: 76 }, yellow: { min: 81, max: 86 }, green: { min: 91, max: 96 } };
...
function createDefaultUserList() {
    return COLORS.map(function (color) {
        return { color: color, state: color === 'red' ? 'normal' : 'computer' };
    });
}
...
function getNextColor(currentColor) {
    var idx = COLORS.indexOf(currentColor);
...
function calcFinishChannelEntry(currentCoordId, steps, color) {
    var finish = FINISH_CHANNEL[color];
    var stepCount = 0;
    ...
    var base = { red: 60, blue: 70, yellow: 80, green: 90 }[color];
    var targetId = base + steps;
    if (targetId > finish.max) {
        // bounce back
        var overshoot = targetId - finish.max;
...
function isWinCell(coordId) {
    var coord = selectCoordValue(coordId);
