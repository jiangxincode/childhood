...
function makeState(board, currentTeam, opts = {}) {
  return {
    ...
    capturedDragon: opts.capturedDragon || [], capturedTiger: opts.capturedTiger || [],
...
describe('constants - 常量定义', () => {
  it('DRAGON_PIECES 应包含8个龙队棋子', () => {
    ...
    expect(DRAGON_PIECES).toEqual(['龙王', '神龙', '金龙', '青龙', '赤龙', '白龙', '风雨龙', '变形龙']);
  });

  it('TIGER_PIECES 应包含8个虎队棋子', () => {
    expect(TIGER_PIECES).toHaveLength(8);
    expect(TIGER_PIECES).toEqual(['虎王', '东北虎', '大头虎', '下山虎', '绿虎', '妖虎', '白虎', '小王虎']);
  });
...
describe('hasAnyLegalAction - 合法操作检测', () => {
  ...
  it('己方牌被完全包围且无吃牌机会且无未翻开的牌时返回 false', () => {
    ...
    const board = [
      ...
      [makeCard('虎王', 'tiger'),  makeCard('青龙', 'dragon'), makeCard('赤龙', 'dragon'), makeCard('虎王', 'tiger')],
      [makeCard('虎王', 'tiger'),  makeCard('虎王', 'tiger'),  makeCard('虎王', 'tiger'),  makeCard('虎王', 'tiger')]
    ];
    expect(hasAnyLegalAction(board, 'dragon')).toBe(false);
  });
